...
# shellcheck source=./scripts/start-kasm-server.sh
source "$SCRIPT_DIR/scripts/start-kasm-server.sh"
...
echo "============================================"
echo "🚀 Antigravity KasmVNC Launcher (with VPN)"
echo "============================================"
echo ""
...
echo "   which vncserver: $(command -v vncserver 2>/dev/null || echo 'NOT FOUND')"
...
# Keep PID file format compatible with existing status/stop scripts.
...
echo "🔄 Keeping alive (tailing KasmVNC logs)..."
tail -f "$HOME/.vnc/"*":${DISPLAY_NUM}.log" 2>/dev/null || sleep infinity
